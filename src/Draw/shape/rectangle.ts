import { CallbackProperty, Entity, JulianDate, Rectangle as CesiumRectangle, Cartesian3, Cartesian2 } from 'cesium';
...
export default class Rectangle extends BasicGraphices implements LifeCycle {
  ...
  playOff(): Entity {
    ...
    const rect = this.result.rectangle?.coordinates?.getValue(new JulianDate()) as TRectangle;
    const { west, east, north, south } = rect;
    const positions = [[west, north], [east, north], [east, south], [west, south]].map(pos => Cartesian3.fromRadians(pos[0], pos[1], 0));
