...
import type { LabelEntityStyle } from '../_config/entityTyping';
import type { SymbolStyle } from '../_config/typing';
...
const symbolConfig2Style = (config: SymbolStyle) => {
  const style: LabelEntityStyle = {
    ...
    paint: {
      ...
      pixelOffset: new Cartesian2(config['text-offsetX'] ?? 0, config['text-offsetY'] ?? -10),
    },
  };
  return style;
};
