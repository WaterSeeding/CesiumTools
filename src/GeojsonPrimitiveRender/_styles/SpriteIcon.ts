import { getSpriteJson } from "../_utils/getSpriteJson";
import { image2canvas, loadImage } from "./renderTool";

import type { SpriteJson } from "../_utils/getSpriteJson";
    ...
    if (optionsCache[key]) options = optionsCache[key];
      ...
      options = {
        spriteImage: await loadImage(
          sprite.url.replace(/(.*)\/sprite/, "$1/sprite.png"),
          sprite.params
        ),
        json: await getSpriteJson(
          sprite.url.replace(/(.*)\/sprite/, "$1/sprite.json"),
          sprite.params
        ),
      };
